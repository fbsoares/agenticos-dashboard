...
resource "coder_script" "install_ai_agents" {
  agent_id           = coder_agent.main.id
  display_name       = "Install AI Agents"
  icon               = "/icon/code.svg"
  run_on_start       = true
  start_blocks_login = true
  script             = replace(file("${path.module}/install-ai-agents.sh"), "\r\n", "\n")
...
resource "coder_script" "clone_repos" {
  ...
  run_on_start       = true
  start_blocks_login = true
  script             = replace(file("${path.module}/clone-repos.sh"), "\r\n", "\n")
  depends_on         = [coder_script.install_languages]
}
